...
//    private Logger logger = LoggerFactory.getLogger(MovieRestApi.class);
    ...
    public Model getGuessMovies(@RequestParam("username")String username,@RequestParam("num")int num, Model model) {
        ...
        if(recommendations.size()==0){
            if(user.getPrefGenres().size()>0){
                String randomGenres = user.getPrefGenres().get(0);
                ...
                String randomGenres = "Action";
                recommendations = recommenderService.getTopGenresRecommendations(new TopGenresRecommendationRequest(randomGenres,num));
            }
    ...
    public Model rateToMovie(@PathVariable("id")int id,@RequestParam("score")Double score,@RequestParam("username")String username, Model model) {
        ...
        if(complete) {
            ...
            String log = user.getUid() +"|"+ id +"|"+ request.getScore() +"|"+ System.currentTimeMillis()/1000;
            logger.info(Constant.MOVIE_RATING_PREFIX + ":" +log);
            kafkaTemplate.send("recommender",log);
        }
        model.addAttribute("success",true);
